...
function setNestedAttribute(obj, attrList, newValue) {
	if (attrList.length === 1) {
		obj[attrList[0]] = newValue;
	} else {
		const currentAttr = attrList[0];
		if (obj[currentAttr] === undefined) {
			obj[currentAttr] = {};
		}
		setNestedAttribute(obj[currentAttr], attrList.slice(1), newValue);
	}
}
		...
		for(const [key2, value2] of Object.entries(value.sections)) {
			...
			for(const [key3, value3] of Object.entries(value.sections[key2])) {
				...
				value_input.addEventListener('change', function (event) {
					...
					setNestedAttribute(value, value_name, inputValue);
				});
...
function LoadObjectWithObject(gameObject, target_name) {
	UnloadObjects();
